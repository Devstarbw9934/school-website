<?php 
		...
		$queryV = "SELECT * FROM svideos WHERE eid='$eid'"; 
		$resltV = mysqli_query($conn,$queryV);
		$mark=0;
			$query = "SELECT * FROM squestions WHERE eid='$eid'"; 
...
<thead><tr><th>Video</th><th>Training</th></tr></thead>
<tbody>
<?php
while($rowV = mysqli_fetch_array($resltV)){   //Creates a loop to loop through results
					$vid=$rowV['vid'];
							$result = mysqli_query($conn,"SELECT  id,subject,name FROM videos WHERE id=$vid;");
							$row = mysqli_fetch_row($result);
...
<div class="row"><div class="col-md-12"><div class="table-responsive">
<table class="table table-hover">
<thead><tr><th>Question</th><th>Training</th><th>Marks</th><th>Hint</th></tr></thead>
<tbody>
<?php
while($row = mysqli_fetch_array($reslt)){   //Creates a loop to loop through results
					$qid=$row['qid'];
							...
							<td>" . $row[3] . "</td>
							<td>";
							$querys ="select * from subjects where id ='".$row[1]."'";
							$results = mysqli_query($conn,$querys);
							while($rows = mysqli_fetch_array($results)){
								echo $rows['subject'];
							}
							echo "</td><td>" . $row[2] . "</td><td>" . $row[4] . "</td></tr>"; 						
						}
?>
</tbody>
</table>

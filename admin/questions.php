...
<?php
					while($row = mysqli_fetch_array($result)){   //Creates a loop to loop through results
						if ($row['is_arabic'] == 'n') {
							echo "<tr><td>" . $row['id'] . "</td><td>" . $row['question'] . "</td><td>" . $row['marks'] . "</td><td>";
							...
							<a class='btn btn-primary' href='new_question.php?id=" .$row['id'] . "'><i class='fa fa-pencil-square-o' data-toggle='tooltip'></i>Edit</a>
							...
							echo "<tr><td>" . $row['id'] . "</td><td style='direction:rtl;'>" . $row['question'] . "</td><td>" . $row['marks'] . "</td><td>";
							$querys ="select * from subjects where id ='".$row['subject']."'";
							$results = mysqli_query($conn,$querys);
							while($rows = mysqli_fetch_array($results)){
								echo $rows['subject'];
